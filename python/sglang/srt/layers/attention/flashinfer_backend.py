...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        if not self.forward_metadata.use_ragged:
            ...
            # Compute causal flag (DLLM needs bidirectional during denoising)
            causal = not layer.is_cross_attention
            if self.is_dllm_model and layer.attn_type == AttentionType.ENCODER_ONLY:
                # DLLM bidirectional by default, causal only for:
                #  - prompt prefill (ForwardMode.EXTEND, not DLLM_EXTEND)
                #  - final KV-update pass (dllm_causal_kv_update=True)
                if (
                    self.dllm_config is not None
                    and self.dllm_config.causal_context
                    and (
            ...
            if layer.is_cross_attention:
                causal = False
            elif layer.attn_type == AttentionType.ENCODER_ONLY:
                # DLLM bidirectional attention — but with causal_context=True:
                #   • STAGING_PREFILL (ForwardMode.EXTEND) must use causal attention
                #     so the prompt KV matches HF's causal_context=True pass.
                #   • The final KV-update pass inside FastDiffuser (flagged via
                #     dllm_causal_kv_update) must also use causal attention, matching
                #     HF's per-block KV-update with use_causal_mask=True.
                #   • All other DLLM_EXTEND denoising steps use bidirectional.
                if (
                    self.is_dllm_model
                    ...
                    causal = True   # causal KV for prompt cache + final KV-update
                else:
                    causal = False
            ...
            if self.forward_metadata.extend_no_prefix:
                ...
                if self.is_dllm_model:
                    ...
                    if "enable_pdl" in _inspect.signature(
