# coding=utf-8
"""
SGLang native implementations of Nemotron Diffusion LMs.

Two architectures:
  - DiffEncoderModel      : Qwen3-based  (nvidia/Nemotron-Diffusion-Research-8B-v0)
  - MinistralDiffEncoderModel : Ministral-based (nvidia/Nemotron-Diffusion-Exp-Ministral-8B)
...
class QwenDiffuserAttention(nn.Module):
    """
    Bidirectional attention for the Qwen3-based Nemotron diffusion model.
...
class DiffEncoderModel(nn.Module):
    """
    Qwen3-based Nemotron Diffusion LM (nvidia/Nemotron-Diffusion-Research-8B-v0).
...
# ---------------------------------------------------------------------------
# MinistralDiffEncoderModel (Ministral-based)
# ---------------------------------------------------------------------------
...
def _llama4_q_scale(
    ...
    return (
        1.0
        + beta
        * torch.log(1.0 + torch.floor(positions.float() / max_pos))
    ).unsqueeze(-1)


class MinistralDiffuserAttention(nn.Module):
    """
    Bidirectional attention for the Ministral-based Nemotron diffusion model.
...
class MinistralDiffuserLayer(nn.Module):
    def __init__(
        self,
        config: PretrainedConfig,
        layer_id: int,
...
class MinistralDiffuserModel(nn.Module):
    def __init__(
        self,
        config: PretrainedConfig,
        quant_config: Optional[QuantizationConfig] = None,
...
class MinistralDiffEncoderModel(nn.Module):
    """
    Ministral-based Nemotron Diffusion LM (nvidia/Nemotron-Diffusion-Exp-Ministral-8B).

    HF architecture name: MinistralDiffEncoderModel
    mask_token_id: 100
    """
    ...
    def __init__(
        ...
        self.model = MinistralDiffuserModel(
            config,
            quant_config,
            prefix=add_prefix("model", prefix),
        )
...
EntryClass = [DiffEncoderModel, MinistralDiffEncoderModel]
